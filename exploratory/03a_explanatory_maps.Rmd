...
data <- dens_grid_geom %>% filter(builtup2011 == 1) %>% 
  mutate(oac_constrained = as.numeric(ifelse(SPRGRP == "7",1,0)),
         ...
         addresses_2013 = if_else(is.na(addresses_2013), 0, addresses_2013)) %>% 
  select(amenity_count:deprivation, p_pre1919:oac_suburban, addresses_2013) %>% 
...
```{r, echo=FALSE, warning=FALSE}
var_name <- "Address density"
unit <- " "

dataset_new <- compute_fisher(data, addresses_2013)
...
data$deprivation_inv <- 11 - data$deprivation
dataset_new <- compute_fisher(data, deprivation_inv)
...
all_maps_02 <- wrap_plots(
  plot_sfh_share, plot_addresses_2013,
