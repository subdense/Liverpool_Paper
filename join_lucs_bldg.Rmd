...
# Knit from the project root. Open Liverpool_Paper.Rproj first so the working directory is set.
```
...
dat_13 <- read.csv("Input_Data/LUCS/LUCS_2013-14_addresses.csv")
dat_14 <- read.csv("Input_Data/LUCS/LUCS_2014-15_addresses.csv")
dat_15 <- read.csv("Input_Data/LUCS/LUCS_2015-16_addresses.csv")
dat_16 <- read.csv("Input_Data/LUCS/LUCS_2016-17_addresses.csv")
dat_17 <- read.csv("Input_Data/LUCS/LUCS_2017-18_addresses.csv")

## data 2018-2021 different structure
dat_18 <- read.csv("Input_Data/LUCS/LUCS_2018-19_addresses.csv")
dat_19 <- read.csv("Input_Data/LUCS/LUCS_2019-20_addresses.csv")
dat_20 <- read.csv("Input_Data/LUCS/LUCS_2020-21_addresses.csv")
dat_21 <- read.csv("Input_Data/LUCS/LUCS_2021-22_addresses.csv")

## Boundary Liverpool
liv <- st_read("Input_Data/Liverpool Metropolitan Boundary/liverpool_metropolitan_dissolved.gpkg")

## Boundary Bristol
#NOTE: Bristol boundary not in repo. Expected location below.
bri <- st_read("Input_Data/bcr-lad-avon.gpkg")

## Building footprints t1
#NOTE: OS MasterMap footprints not in repo. Expected locations below.
bldg_23 <- st_read("Input_Data/OS MasterMap/footprints_2023.gpkg")
bldg_23_bri <- st_read("Input_Data/OS MasterMap/footprints_2023_bristol.gpkg")
```
...
## write data
```{r}
st_write(joined, "R Output/lucs_liv.gpkg", delete_layer = TRUE)
st_write(joined_bri, "R Output/lucs_bri.gpkg", delete_layer = TRUE)
```
